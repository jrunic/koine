...
- **Lógica nova dentro de `atualizar.aplicar()` só entra em vigor no `atualizar` SEGUINTE, em qualquer SO — não é peculiaridade do Windows.** No POSIX, `aplicar()` roda in-process, no interpretador que já tinha o módulo antigo carregado antes do swap do pyz no disco — trocar o arquivo não recarrega o código em memória. No Windows é mais visível porque o mecanismo é explícito: o "finalizador" que aplica a atualização é uma **cópia do pyz que está saindo**, de propósito, para suportar downgrade (`cli.py::_cmd_atualizar`, comentário no bloco `if sys.platform == "win32"`). Mesma causa da #749 (lista de wrappers montada com o código da versão que estava saindo). Medido ao vivo na v0.18.0: `agente.migrar_default_inequivoco`, chamado de dentro de `atualizar.aplicar()`, não disparou no `atualizar` que introduziu a função — só no próximo. Regra prática: lógica nova em `aplicar()` cuja correção o release **anuncia** só é real a partir do ciclo seguinte; se o efeito precisa valer já no release que o introduz, delegar ao pyz novo via subprocesso (padrão de `_gerar_wrappers_pelo_novo`), não chamar inline.
