...
3. **Wrappers de cliente** — `koine` + `kn-*` em `~/.local/bin/`, invocando o Python detectado na instalação; **no Windows, a pasta entra no `Path` do seu usuário** logo depois, por API do registro — só a entrada do Koine, sem tocar no que é de outros programas
